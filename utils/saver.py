...
class CheckpointSaver():
    ...
    def exists_checkpoint(self, checkpoint_file=None):
        ...
        if checkpoint_file is None:
            print('CHECKPOINT FILE IS NONE')
            return False if self.latest_checkpoint is None else True
        else:
            print(os.path.isfile(checkpoint_file))
            return os.path.isfile(checkpoint_file)
    
    def save_checkpoint(self, models, optimizers, epoch, batch_idx, batch_size, dataset_perm, total_step_count, is_update=True, eval_stats=None):
        """Save checkpoint."""
        ...
        if is_update:
            ...
            checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'previous_{}_{}_{:.1f}_{:.1f}'.format(epoch, batch_idx, eval_stats['mpjpe'], eval_stats['pa_mpjpe']) + '.pt'))
        checkpoint = {}
        for model in models:
