...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        self.model = hmr(config.SMPL_MEAN_PARAMS, pretrained=True, name='hmr_vit', bbox_type=self.options.bbox_type, encoder=self.options.encoder).to(self.device)
        ...
        if self.options.train_dataset == 'old agora':
            ...
            self.smpl_male = SMPL_agora(config.SMPL_MODEL_DIR,
                     gender='male',
                     batch_size=self.options.batch_size,
                            create_transl=False).to(self.device)
            self.smpl_female = SMPL_agora(config.SMPL_MODEL_DIR,
                       gender='female',
                       batch_size=self.options.batch_size,
                            create_transl=False).to(self.device)
            ...
            self.smpl_female = SMPL(config.SMPL_MODEL_DIR,
                       ...
                       batch_size=self.options.batch_size,
                            create_transl=False).to(self.device)
            self.smpl_male_kid = SMPL(config.SMPL_MODEL_DIR, model_type='smpl',
                                      gender='male', age='kid',
                                      kid_template_path=config.SMPL_KID_TEMP_DIR,
                                      batch_size=self.options.batch_size,
                                      create_transl=False).to(self.device)
            self.smpl_neutral_kid = SMPL(config.SMPL_MODEL_DIR, model_type='smpl',
                                      gender='neutral', age='kid',
                                      kid_template_path=config.SMPL_KID_TEMP_DIR,
                                      batch_size=self.options.batch_size,
                                      create_transl=False).to(self.device)
    ...
    def compute_keypoints2d_loss_cliff(
        ...
        if viz:
            ...
            # cv2.waitKey()
        # print(pred_keypoints2d_bbox, gt_keypoints2d)
        ...
        # scale_w=scale_w.unsqueeze(-1)
        # # print(scale_w.shape)
        # pred_keypoints2d_bbox[:, :, 0] = (2. * pred_keypoints2d_bbox[:, :, 0] / self.crop_w - 1.)*scale_w
        # gt_keypoints2d[:, :, 0] = (2. * gt_keypoints2d[:, :, 0] / self.crop_w - 1.)*scale_w
        # pred_keypoints2d_bbox[:, :, 1] = (2. * pred_keypoints2d_bbox[:, :, 1] / self.crop_h - 1.)*scale_w
        # gt_keypoints2d[:, :, 1] = (2. * gt_keypoints2d[:, :, 1] / self.crop_h - 1.)*scale_w
    ...
    def shape_loss(self, pred_vertices, gt_vertices, valid_vert, dataset=None, viz=False, dbg_dataset='3dpw'):
        """Compute per-vertex loss on the shape for the examples that SMPL annotations are available."""
        pred_vertices_with_shape = pred_vertices[valid_vert == 1]
        gt_vertices_with_shape = gt_vertices[valid_vert == 1]
        # print(len(gt_vertices_with_shape))
        ...
        if len(gt_vertices_with_shape) > 0:
            return self.criterion_shape(est_vertices_aligned, target_vertices_aligned)
        else:
            # print('No valid verts')
            return torch.FloatTensor(1).fill_(0.).to(self.device)

    def smpl_losses(self, pred_rotmat, pred_betas, gt_pose, gt_betas, has_smpl, valid_beta, dataset=None):
    ...
    def train_step(self, input_batch, cur_epoch, cur_step):
        ...
        valid_vert = input_batch['valid_vert'].byte()
        valid_beta = input_batch['valid_beta'].byte()
        ...
        if self.options.train_dataset == 'agora':
            isKid = input_batch['isKid']
            # print(input_batch['occlusion'][0]/100.)
            occls_rate = (1.-(input_batch['occlusion']/100.)+1e-2)
            # print(occls_rate)
            scale_w = input_batch['scale_weight']
        else:
            occls_rate = torch.ones((batch_size), device=self.device)
        


        # Get GT vertices and model joints
        # Note that gt_model_joints is different from gt_joints as it comes from SMPL
        gt_out = self.smpl(betas=gt_betas[:, :10], body_pose=gt_pose[:, 3:], global_orient=gt_pose[:, :3])
        ...
        # gt_vertices_male_kid = self.smpl_male_kid(global_orient=gt_pose[:,:3], body_pose=gt_pose[:,3:], betas=gt_betas[:, :11]).vertices 
        # gt_vertices_neutral_kid = self.smpl_neutral_kid(global_orient=gt_pose[:,:3], body_pose=gt_pose[:,3:], betas=gt_betas[:, :11]).vertices  
        # print(gender)
        ...
        loss_regr_pose, loss_regr_betas = self.smpl_losses(pred_rotmat, pred_betas, gt_pose, gt_betas[:, :10], has_smpl, dataset=dataset_name, valid_beta=valid_beta)
        ...
        loss_keypoints_3d = self.keypoint_3d_loss(pred_joints[:, self.joints_idx:], gt_joints, has_pose_3d, dataset=dataset_name, viz=self.options.viz_debug, dbg_dataset=self.options.debug_dataset)
        ...
        loss_shape = self.shape_loss(pred_vertices, gt_vertices, valid_vert=valid_vert, dataset=dataset_name, viz=self.options.viz_debug, dbg_dataset=self.options.debug_dataset)
    ...
    def test(self, epoch):
        ...
        if self.options.eval_dataset != 'agora':
            mpjpe, pa_mpjpe, pve = run_evaluation(self.model, '3dpw', self.eval_dataset, None,
                                batch_size=512,
                                shuffle=False,
                                log_freq=50, 
            ...
            mpjpe, pa_mpjpe, pve = run_evaluation_agora(self.model, 'agora', self.eval_dataset, None,
                                batch_size=512,
                                shuffle=False,
                                log_freq=50, 
                                with_train=True, eval_epoch=epoch, summary_writer=self.summary_writer,
    ...
    def train_summaries(self, input_batch, output, losses):
        ...
        for idx in img_to_sum:
            ...
            images_pred = renderer.visualize_tb(pred_vertices[idx].unsqueeze(0), gt_vertices[idx].unsqueeze(0), pred_cam_t[idx].unsqueeze(0),
                                                image_full, grid=True)
            images_gt = renderer.visualize_tb(gt_vertices[idx].unsqueeze(0), pred_vertices[idx].unsqueeze(0), pred_cam_t[idx].unsqueeze(0),
                                                image_full, grid=True, base_color=(0.1, 0.9, 0.1, 1.0))
            images_crop_pred = crop_renderer.visualize_tb(pred_vertices[idx].unsqueeze(0), gt_vertices[idx].unsqueeze(0), pred_cam_crop[idx].unsqueeze(0), images[idx:idx+1], grid=True)
            images_crop_gt = crop_renderer.visualize_tb(gt_vertices[idx].unsqueeze(0), pred_vertices[idx].unsqueeze(0), pred_cam_crop[idx].unsqueeze(0), images[idx:idx+1], grid=True, base_color=(0.1, 0.9, 0.1, 1.0))
            
            self.summary_writer.add_image('pred_mesh_{}_{}'.format(dataset[idx], num), images_pred, self.step_count)
            self.summary_writer.add_image('gt_mesh_{}_{}'.format(dataset[idx], num), images_gt, self.step_count)
            self.summary_writer.add_image('pred_crop_mesh_{}_{}'.format(dataset[idx], num), images_crop_pred, self.step_count)
            self.summary_writer.add_image('gt_crop_mesh_{}_{}'.format(dataset[idx], num), images_crop_gt, self.step_count)
