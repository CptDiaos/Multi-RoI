...
from utils.geometry import batch_rodrigues, perspective_projection, estimate_translation, cam_crop2full, j2d_processing_torch
...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        self.smplify = SMPLify(step_size=1e-2, batch_size=self.options.batch_size, num_iters=self.options.num_smplify_iters, focal_length=self.render_focal_length)
        ...
        # Create renderer
        self.renderer = Renderer(focal_length=self.render_focal_length, img_res=self.options.img_res, faces=self.smpl.faces)
    ...
    def compute_keypoints2d_loss_cliff(
            ...
            gt_keypoints2d_full: torch.Tensor,
        ...
        gt_keypoints2d_bbox = torch.zeros_like(gt_keypoints2d_full, device=device)
        for b_ind in range(pred_keypoints2d_bbox.shape[0]):
            pred_keypoints2d_bbox[b_ind] = j2d_processing_torch(pred_keypoints2d_full[b_ind], crop_center[b_ind], crop_scale[b_ind], r=0, f=is_flipped[b_ind], crop_t=trans[b_ind])
            # gt_keypoints2d_bbox[b_ind] = j2d_processing_torch(gt_keypoints2d_full[b_ind], crop_center[b_ind], crop_scale[b_ind], r=0, f=is_flipped[b_ind], crop_t=trans[b_ind])
        # pred_keypoints2d_norm = 2. * pred_keypoints2d / img_size - 1.
        # gt_keypoints2d_norm = 2. * gt_keypoints2d / img_size - 1.
        loss = self.keypoint_loss(pred_keypoints2d_bbox, gt_keypoints2d.float(),
    ...
    def keypoint_loss(self, pred_keypoints_2d, gt_keypoints_2d, openpose_weight, gt_weight, scale=None):
        ...
        # if torch.all(scale):
        #     s = 0.5 / (200. * scale).clone()
        #     s = s.unsqueeze(-1).expand(-1, 49)
    ...
    def train_step(self, input_batch):
        ...
        crop_tran = input_batch['crop_tran']
        ...
        gt_keypoints_2d_orig[:, :, :-1] = 0.5 * self.options.img_res * (gt_keypoints_2d_orig[:, :, :-1] + 1)
        gt_cam_t = estimate_translation(gt_model_joints, gt_keypoints_2d_orig, focal_length=np.array(focal_length.detach().cpu(), dtype=int), img_size=self.options.img_res)
        ...
        pred_cam_crop = torch.stack([pred_camera[:, 1],
                                     pred_camera[:, 2],
                                     2 * self.render_focal_length / (self.options.img_res * pred_camera[:, 0] + 1e-9)], dim=-1)
        ...
        output = {'pred_vertices': pred_vertices.detach(),
                  ...
                  'dataset_name': dataset_name.detach(),
                  # 'opt_cam_t': opt_cam_t}
                  'opt_cam_t': None}
    ...
    def train_summaries(self, input_batch, output, losses):
        ...
        pred_cam_t = output['pred_cam_crop']
