...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        # self.train_ds = MixedDataset(self.options, ignore_3d=self.options.ignore_3d, use_augmentation=True, is_train=True)
        self.train_ds = BaseDataset(self.options, 'coco', ignore_3d=self.options.ignore_3d, use_augmentation=True, is_train=True, 
                                    use_extraviews=self.options.use_extraviews, large_bbx=self.options.large_bbx)
        self.eval_dataset = BaseDataset(None, '3dpw', is_train=False, use_extraviews=self.options.use_extraviews, large_bbx=self.options.large_bbx)

        self.model = hmr(config.SMPL_MEAN_PARAMS, pretrained=True, name='hmr_fuseviews').to(self.device)
    ...
    def camera_losses(self, crop_cam, center, scale, full_img_shape):
        ori_b = center.shape[0]//self.nviews
        img_h, img_w = full_img_shape[:, 0], full_img_shape[:, 1]
        cx, cy, b = center[:, 0], center[:, 1], scale * 200
        w_2, h_2 = img_w / 2., img_h / 2.
        bs = b * crop_cam[:, 0] + 1e-9
        main_ind = torch.full((center.shape[0],), 0) + self.nviews*torch.arange(ori_b).unsqueeze(1).repeat(1, self.nviews).view(-1,)
        views_ind = torch.arange(self.nviews).unsqueeze(0).repeat(ori_b, 1).view(-1,) + self.nviews*torch.arange(ori_b).unsqueeze(1).repeat(1, self.nviews).view(-1,)
        ...
        return self.criterion_regularize(reg_x+reg_y, torch.zeros_like(reg_x, device=reg_x.device))
    ...
    def train_step(self, input_batch, cur_epoch, cur_step):
        ...
        gt_joints = input_batch['pose_3d'].unsqueeze(1).repeat(1, n_views, 1, 1).view(batch_size*n_views, 24, 4)  # 3D pose
        ...
        gt_keypoints_2d_extra = input_batch['keypoints_extra']
        ...
        gt_keypoints_2d = torch.cat([gt_keypoints_2d.unsqueeze(1), gt_keypoints_2d_extra], 1).view(batch_size*n_views, 49, 3)
        ...
        pred_cam_crop = torch.stack([pred_camera[:, 1],
                                     pred_camera[:, 2],
                                     2 * self.render_focal_length / (self.options.img_res * pred_camera[:, 0] + 1e-9)],
                                    dim=-1)
        ...
        loss_cam = self.camera_losses(pred_camera, center, scale, full_img_shape)
        ...
        loss = self.options.shape_loss_weight * loss_shape + \
               ...
               self.options.pose_loss_weight * loss_regr_pose + self.options.beta_loss_weight * loss_regr_betas + loss_cam
        loss *= 60
    ...
    def test(self, epoch):
        ...
        mpjpe, pa_mpjpe = run_evaluation(self.model, '3dpw', self.eval_dataset, None,
                            ...
                            shuffle=False,
                            log_freq=50, 
                            with_train=True, eval_epoch=epoch, summary_writer=self.summary_writer, use_extra=self.options.use_extraviews, use_fuse=True)
        results = {'mpjpe': mpjpe,
                   'pa_mpjpe': pa_mpjpe}
        return results
    ...
    def train_summaries(self, input_batch, output, losses):
        ...
        if self.options.use_extraviews:
            ...
            images_extra = images_extra * torch.tensor([0.229, 0.224, 0.225], device=images.device).reshape(1, 3, 1, 1).repeat(1, 4, 1, 1)
            images_extra = images_extra + torch.tensor([0.485, 0.456, 0.406], device=images.device).reshape(1, 3, 1, 1).repeat(1, 4, 1, 1)
        ...
        for idx in img_to_sum:
            ...
            images_crop_pred = crop_renderer.visualize_tb(pred_vertices[idx_ext].unsqueeze(0), gt_vertices[idx_ext].unsqueeze(0), pred_cam_crop[idx_ext].unsqueeze(0), images[idx:idx+1], blank_bg=True)
            if self.options.use_extraviews:
                images_extra_list = []
                for i in range(4):
                    image_extra = images_extra[idx:idx+1, 3*i:3*(i+1)]
                    print('image_extra',image_extra.shape)
                    images_crop_aug_pred = crop_renderer.visualize_tb(pred_vertices[idx*self.nviews+i+1].unsqueeze(0), gt_vertices[idx*self.nviews+i+1].unsqueeze(0), pred_cam_crop[idx*self.nviews+i+1].unsqueeze(0), image_extra, blank_bg=True)
